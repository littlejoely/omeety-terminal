...
// 验证：① 帧编解码 ② PTY echo ③ MCP 双协议握手 + tools/list(29) ④ tools/call 中继往返。
...
try {
  ...
  ok(httpList?.tools?.length === 29, `Streamable HTTP tools/list = 29 (实际 ${httpList?.tools?.length})`)
  ...
  ok(list?.result?.tools?.length === 29, `MCP tools/list = 29 (实际 ${list?.result?.tools?.length})`)
