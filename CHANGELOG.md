...
- Omeety now scopes all browser automation to the window its side panel opens in. Snapshots, navigation, clicks, `open_tab`, and the 选取 picker act only on that window; new pages open there too — work in another window is never disturbed. No UI, no toggle: open the side panel in a window and omeety works there, period. The side panel auto-reconnects on SW restart and re-reports its window. With no panel reporting a window it falls back to the old "follow focus" behavior.
- omeety 现在只在「侧栏所在窗口」干活——快照、导航、点击、`open_tab`、选取都只作用该窗口，新页面也只开到该窗口，另一个窗口的工作不受打扰。无按钮、无切换：侧栏开在哪，omeety 就在哪干。侧栏断线/SW 重启会自动重连并重新上报所在窗口。没有任何侧栏上报窗口时回退到原来的「跟随焦点」行为。
...
- Reconnect backoff now actually accumulates instead of being reset on every attempt (the side panel auto-reconnects on SW restart and re-reports its window).
- 侧栏断线自动重连的指数退避现在能真正累加（之前每次尝试都被重置成 500ms），SW 重启后会自动重连并重新上报所在窗口。
